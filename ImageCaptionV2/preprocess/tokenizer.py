...
OUTPUT_DIR = r'F:\Dataset\flickr30k_dict'
seed = 123456789
random.seed(seed)
...
def generate_training_token(captions, txt, max_caption_per_img=5):
...
def gen_eval_set(split, name):
    ...
    for filename, cap in captions:
        filename2cap[filename].append([w.lower() for w in cap[1:-1]])
...
def main(mx_train=None):
    ...
    print('训练集大小:{}'.format(len(captions)))
    print('字典大小:{}\n句子最大长度:{}\n<start> idx:{}\n<stop> idx:{}'.format(
